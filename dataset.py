...
class ToFDataset(Dataset):
    def __init__(self, root_dir, train: bool = True, transforms=None, isComplexModel=True):
        assert os.path.exists(root_dir), f'{root_dir} does not exist'
        self.isComplexModel = isComplexModel
       ####exp data load
        # if train:
...
####syn data load        
    ...
    def __getitem__(self, idx):
        ...
        clear_tof_amp_npy = np.where(clear_tof_amp_npy == 0, 0.1, clear_tof_amp_npy)
        fog_tof_amp_npy = np.where(fog_tof_amp_npy == 0, 0.1, fog_tof_amp_npy)
        ...
        if self.isComplexModel:
            ...
            input_fog = complex_fog.unsqueeze(0)
            output_residual = residual.unsqueeze(0)
            #size: (1, 480, 640)
        else:
            x_depth = x_phase * 299792458 / (4 * np.pi * 20 * 1000)
            y_depth = y_phase * 299792458 / (4 * np.pi * 20 * 1000)
        
            input_fog = torch.stack((x_amp, x_depth), dim=0)
            
            output_residual_depth = x_depth - y_depth
            output_residual_amp = x_amp - y_amp
            output_residual = torch.stack((output_residual_amp, output_residual_depth), dim=0)
            # output_residual = output_residual.unsqueeze(0)
        
        if self.transforms:
            input_fog, output_residual = self.transforms(input_fog, output_residual)
            # print(intput.size(), output.size())   
        return input_fog, output_residual
